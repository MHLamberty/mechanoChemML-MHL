...
def principal_stretches_from_C(C):
    """
    Analytic principal stretches for a symmetric 3x3 tensor C using
    invariant-based cubic eigenvalue formula.

    Returns lam1, lam2, lam3 where lam_i = sqrt(eig_i(C)).
    """
    eps = Constant(1e-12)
    I = Identity(C.ufl_shape[0])

    # Mean part
    q = tr(C) / 3.0

    # Deviatoric part of C relative to q*I
    B = C - q * I

    # p from Frobenius norm of deviatoric part
    p2 = tr(B * B) / 6.0
    p = sqrt(p2 + eps)

    # Dimensionless quantity for cubic solution
    r = det(B) / (2.0 * p**3 + eps)

    # Clamp r into [-1, 1] for acos stability
    r_clamped = conditional(lt(r, -1.0), -1.0,
                 conditional(gt(r,  1.0),  1.0, r))

    phi = acos(r_clamped) / 3.0

    # Eigenvalues of symmetric 3x3 tensor
    ...
    # Protect against tiny negative values from roundoff
    eig1 = conditional(lt(eig1, eps), eps, eig1)
    eig2 = conditional(lt(eig2, eps), eps, eig2)
    eig3 = conditional(lt(eig3, eps), eps, eig3)
...
def W_neo_hookean(F, J, I1bar, params):
    """
    Neo-Hookean written in principal stretches.
    ...
    C = F.T * F
    lam1, lam2, lam3 = principal_stretches_from_C(C)
    ...
    Wiso = 0.5 * mu * (lam1_bar**2 + lam2_bar**2 + lam3_bar**2 - 3.0)

    return Wvol + Wiso

def W_arruda_boyce(F, J, I1bar, params):
    """
    Arruda-Boyce model written using principal stretches
    through I1bar = lam1_bar^2 + lam2_bar^2 + lam3_bar^2.
    ...
    I1bar_ps = lam1_bar**2 + lam2_bar**2 + lam3_bar**2

    Wvol = 0.5 * kappa * (J - 1.0)**2

    Wiso = mu * (
        0.5 * (I1bar_ps - 3.0)
        + (1.0 / (20.0 * N)) * (I1bar_ps**2 - 9.0)
        + (11.0 / (1050.0 * N**2)) * (I1bar_ps**3 - 27.0)
    )

    return Wvol + Wiso

def W_ogden_2term(F, J, I1bar, params):
    kappa, mu1, alpha1, mu2, alpha2 = params


    # Build principal stretches directly here (no helper functions)
    C = F.T * F
    lam1, lam2, lam3 = principal_stretches_from_C(C)
    ...
    Wiso = (
        (2.0 * mu1 / (alpha1**2)) *
        (lam1_bar**alpha1 + lam2_bar**alpha1 + lam3_bar**alpha1 - 3.0)
        +
        (2.0 * mu2 / (alpha2**2)) *
        (lam1_bar**alpha2 + lam2_bar**alpha2 + lam3_bar**alpha2 - 3.0)
    )
...
def sensitivity_test(model_name, model_W, cases, mesh_xdmf, loss_factor2, n_cases=1):
    ...
    elif model_name == "ArrudaBoyce":
        tests = [
            np.array([200.0, 0.5, 5.0]),
            np.array([250.0, 0.5, 5.0]),
            np.array([200.0, 0.8, 5.0]),
            np.array([200.0, 0.5, 10.0]),
        ]

    elif model_name == "Ogden2Term":
        tests = [
            np.array([200.0, 0.5, 2.0, 0.1, 6.0]),
            np.array([250.0, 0.5, 2.0, 0.1, 6.0]),
            np.array([200.0, 0.8, 2.0, 0.1, 6.0]),
            np.array([200.0, 0.5, 3.0, 0.1, 6.0]),
            np.array([200.0, 0.5, 2.0, 0.2, 6.0]),
            np.array([200.0, 0.5, 2.0, 0.1, 8.0]),
        ]

    else:
        raise RuntimeError("Unknown model_name in sensitivity_test: " + str(model_name))
...
def model_init_and_bounds(model_name):
    ...
    if model_name == "ArrudaBoyce":
        # params = [kappa, mu, N]
        x0 = np.array([200.0, 0.5, 5.0], dtype=float)
        bounds = [
            (1e-6, 1000.0),  # kappa
            (1e-6, 10.0),    # mu
            (1.0, 100.0),    # N
    ...
    if model_name == "Ogden2Term":
        # params = [kappa, mu1, alpha1, mu2, alpha2]
        # Start near the ABAQUS truth:
        # mu1 = 0.5, alpha1 = 2, mu2 = 0.1, alpha2 = 6, D1 = 0.01
        # With Wvol = 0.5*kappa*(J-1)^2, a reasonable initial kappa is about 2/D1 = 200
        x0 = np.array([200.0, 0.5, 2.0, 0.1, 6.0], dtype=float)
...
if __name__ == "__main__":
    ...
    cases = [
        # 1) Unconfined compression: measure RF2 on YMAX
        {
            ...
            "inp_path": "HE_Unconfined_0p8.inp",
            "u_csv": "cube_U_HE_Unconfined_0p8.csv",
            "rf_meas": read_rf_component("cube_RF2_HE_Unconfined_0p8.txt", "RF2_top"),
            "meas_face": "YMAX",
        ...
        # 2) Confined compression: measure RF2 on YMAX
        {
            "name": "HE_Confined",
            "bc_mode": "HE_Confined",
            "inp_path": "HE_Confined_0p8.inp",
            "u_csv": "cube_U_HE_Confined_0p8.csv",
            "rf_meas": read_rf_component("cube_RF2_HE_Confined_0p8.txt", "RF2_top"),
            "meas_face": "YMAX",
            "load_dir": (0.0, 1.0, 0.0),
        },
        ...
        # 3) Plane strain (still loaded in Y): measure RF2 on YMAX
        {
            ...
            "inp_path": "HE_PlaneStrain_0p8.inp",
            "u_csv": "cube_U_HE_PlaneStrain_0p8.csv",
            "rf_meas": read_rf_component("cube_RF2_HE_PlaneStrain_0p8.txt", "RF2_top"),
            "meas_face": "YMAX",
            "load_dir": (0.0, 1.0, 0.0),
        },
        ...
        # 4) Simple shear in X: measure RF1 on YMAX
        {
            ...
            "inp_path": "HE_SimpleShear_0p8.inp",
            "u_csv": "cube_U_HE_SimpleShear_0p8.csv",
            "rf_meas": read_rf_component("cube_RF1_HE_SimpleShear_0p8.txt", "RF1_top"),
            "meas_face": "YMAX",
        ...
        # If you're only fitting using RF2 on YMAX, do this:
        {
            ...
            "inp_path": "HE_BiaxialStretch_0p8.inp",
            "u_csv": "cube_U_HE_BiaxialStretch_0p8.csv",
            "rf_meas": read_rf_component("cube_RF2_HE_BiaxialStretch_0p8.txt", "RF2_top"),
            "meas_face": "YMAX",
            "load_dir": (0.0, 1.0, 0.0),
        },
        # 5B) Biaxial stretch:
        # If you're only fitting using RF2 on YMAX, do this:
        {
            ...
            "inp_path": "HE_BiaxialStretch_0p8.inp",
            "u_csv": "cube_U_HE_BiaxialStretch_0p8.csv",
            "rf_meas": read_rf_component("cube_RF1_HE_BiaxialStretch_0p8.txt", "RF1_top"),
            "meas_face": "XMAX",
            "load_dir": (1.0, 0.0, 0.0),
        },
    ]
  ...
  # Creates bounds, default lower bound 0, upper bound infinity
   # --- Model initial guesses and bounds (see Section G below) ---
    ...
    x0_ab, b_ab = model_init_and_bounds("ArrudaBoyce")
    x0_og, b_og = model_init_and_bounds("Ogden2Term")

    sensitivity_test("NeoHookean", W_neo_hookean, cases, mesh_xdmf, loss_factor2, n_cases=1)
    sensitivity_test("ArrudaBoyce", W_arruda_boyce, cases, mesh_xdmf, loss_factor2, n_cases=1)
    sensitivity_test("Ogden2Term", W_ogden_2term, cases, mesh_xdmf, loss_factor2, n_cases=1)

    res_nh = fit_model("NeoHookean",      W_neo_hookean,      x0_nh, b_nh, cases, mesh_xdmf, loss_factor2)
    res_ab = fit_model("ArrudaBoyce", W_arruda_boyce, x0_ab, b_ab, cases, mesh_xdmf, loss_factor2)
    res_og = fit_model("Ogden2Term",      W_ogden_2term,      x0_og, b_og, cases, mesh_xdmf, loss_factor2)

    results = [("NeoHookean", res_nh), ("ArrudaBoyce", res_ab), ("Ogden2Term", res_og)]
